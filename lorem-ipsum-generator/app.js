...
loremForm.addEventListener('submit', e => {
  ...
  if (value > length || value < 0 || isNaN(value)) {
    loremText.innerHTML = `<p>${getRandomText()}</p>`;
  } else {
    const tempText = getRandomTexts(value);
...
function getRandomText() {
  const rand = texts[Math.floor(length * Math.random())];
  return rand;
}

function getRandomTexts(number) {
  let text = [];
  for (i = 0; i < number; i ++) {
    text.push(getRandomText());
  }
  return text;
}